...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        ############################################################################
        init_hidden_state, cache_affine = affine_forward(features, W_proj, b_proj)
        word_out, cache_embedding = word_embedding_forward(captions_in, W_embed)

        if self.cell_type == 'rnn':
            rnn_outputs, cache_rnn = rnn_forward(word_out, init_hidden_state, Wx, Wh, b)
        elif self.cell_type == 'lstm':
            rnn_outputs, cache_rnn = lstm_forward(word_out, init_hidden_state, Wx, Wh, b)

        scores, cache_temp_affine = temporal_affine_forward(rnn_outputs, W_vocab, b_vocab)
        loss , dsoftmax = temporal_softmax_loss(scores, captions_out, mask)
      
        # BACKWARD PASS  
        dx, grads['W_vocab'], grads['b_vocab'] = temporal_affine_backward(dsoftmax, cache_temp_affine)
        if self.cell_type == 'rnn':
