...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = i, f, o, g, a, x, prev_h, prev_c, Wx, Wh, b, next_h, next_c
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    #############################################################################
    i, f, o, g, a, x, prev_h, prev_c, Wx, Wh, b, next_h, next_c = cache

    dtanhh = dnext_h * o
    dtanhc = dtanhh * (1 - np.square(np.tanh(next_c)))
    dprev_c = f *(dnext_c + dtanhc)

    dao_sig = dnext_h * np.tanh(next_c)
    dai_sig = (dnext_c + dtanhc) * g
    dag_tan = (dnext_c + dtanhc) * i
    daf_sig = (dnext_c + dtanhc) * prev_c

    dai = i*(1-i) * dai_sig
    daf = f*(1-f) * daf_sig
    dao = o*(1-o) * dao_sig
    dag = (1-np.square(g)) * dag_tan
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    # LSTM forward for T time steps.
    for t in range(T):
        cur_x = x[:, t, :]
        prev_h, prev_c, cache_temp = lstm_step_forward(cur_x, prev_h, prev_c, Wx, Wh, b)
        h[:, t, :] = prev_h
        cache.append(cache_temp)
...
def lstm_backward(dh, cache):
    ...
    #############################################################################
    i, f, o, g, a, x, prev_h, prev_c, Wx, Wh, b, next_h, next_c = cache[0]
    N, T, H = dh.shape
    D, H = Wx.shape
    ...
    for t in reversed(range(T)):
        ...
        # Calculate gradients at this time step.
        dx[:, t, :], dprev_h, dprev_c, dWx_temp, dWh_temp, db_temp = lstm_step_backward(cur_dh, cur_dc, cache[t])
